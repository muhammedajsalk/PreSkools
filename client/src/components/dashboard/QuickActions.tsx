...
import { Card, CardHeader, CardContent, Stack, Avatar, Box, Typography, Badge, CardActionArea } from '@mui/material';
import MenuBookIcon from '@mui/icons-material/MenuBook';
import { brandColors } from './DashboardConfig';
...
interface QuickActionProps {
  actions: { 
    ...
    bgColor: string; 
    badge?: number;
    onClick?: () => void; // <--- Added this optional prop
  }[];
}

export default function QuickActions({ actions }: QuickActionProps) {
  return (
            ...
            <Box key={act.id} sx={{ flex: 1, minWidth: { xs: 'calc(50% - 8px)', sm: 'calc(33.33% - 11px)', lg: 'calc(16.66% - 13px)' } }}>
              ...
              <Card 
                ...
                sx={{ 
                  ...
                  cursor: 'pointer', 
                  transition: 'all 0.3s', 
                  '&:hover': { transform: 'translateY(-4px)', boxShadow: `0 12px 24px ${act.color}20`, borderColor: act.color } 
                }}
              >
                {/* Attach the onClick handler here */}
                <CardActionArea 
                  onClick={act.onClick} 
                  sx={{ p: 2.5, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}
                >
                  <Badge badgeContent={act.badge} color="error">
                    <Avatar sx={{ width: 56, height: 56, bgcolor: act.bgColor, color: act.color, mx: 'auto' }}>
                      {act.icon}
                    </Avatar>
                  </Badge>
